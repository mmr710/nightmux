...
fires for tmux sessions bound to a topic in ~/.tgctl.json; anything else exits
silently. Touching ~/.tgctl-hooked/<session> tells tgctl.py to stop scraping
...
def tailed(sid):
    """True when the status-line sidecar is running, so tgctl reads this itself."""
    return bool(sid) and os.path.exists(
        os.path.join(tgctl.STATE_DIR, f"{sid}.json"))
...
def main():
    ...
    if not topic:
        return
    os.makedirs(tgctl.HOOK_DIR, exist_ok=True)
    open(os.path.join(tgctl.HOOK_DIR, sess), "w").close()  # claim delivery
    if tailed(payload.get("session_id")):
        return  # tgctl reads the same transcript, and reports the tool trace too
    text = last_assistant_text(payload.get("transcript_path", ""))
    if text:
        tgctl.send(cfg, topic, f"✅ {sess}\n{text}", mode="md")
