...
Why nightmux is shaped the way it is. The code carries the same notes as
...
| | |
|---|---|
| `nightmux.py` | the daemon. Polling, commands, tmux injection, scraping, spend accounting, setup |
| `nightmux_state.py` | `statusLine` sidecar. Parks context %, 5h/7d windows, model, transcript path, `$TMUX_PANE` |
| `nightmux_stop.py` | `Stop` hook. Sends the final assistant message as exact text |
| `nightmux_notify.py` | `Notification` hook. Sends permission prompts the instant they appear |

The three small ones import `nightmux` for its config and send helpers, and every
...
|---|---|---|
| `cfg` (memory + `~/.nightmux.json`) | token, chat, allowlist, topic→session bindings | yes |
| `~/.nightmux.offset` | polling offset | yes |
| `~/.nightmux-state/queue.json` | prompts held for a rate-limit reset | yes |
| `~/.nightmux-state/<session_id>.json` | sidecar snapshots: context %, limits, transcript path | as a cache |
| `~/.nightmux-hooked/<session>` | "the Stop hook owns delivery for this session" | as a cache |
| `~/.nightmux-files/` | inbound photos and documents | 7 days |
...
`prune` sweeps `~/.nightmux-files` at 7 days and sidecar snapshots at 1 day, and
...
1. **Transcript tail.** When the sidecar is installed, nightmux knows the JSONL
...
2. **Stop hook.** Delivers the final message directly. Touching
   `~/.nightmux-hooked/<session>` claims delivery, so the scraper stands down for
   `HOOK_FRESH` (15 min) and nothing arrives twice.
3. **Pane scrape.** `tmux capture-pane`, diffed against the last capture by
   common prefix. Always available, and the reason nightmux works with no hooks at
...
recognise a rate-limit banner and the reset time inside it. What nightmux just typed
...
When a session hits its usage limit, nightmux parses the reset time from the banner
...
**tmux, not a subprocess.** nightmux attaches to sessions instead of owning them, so
...
**No timezone library.** Python 3.8 has no `zoneinfo`, so nightmux scopes `TZ` and
